/**
 * SVK Life OS — Service Worker v5.2.1
 ...
 *
 * v5.2.1: added huggingface.co (covers huggingface.co, *.huggingface.co,
 * and cdn-lfs.huggingface.co — all three are already allowed in CSP's
 * connect-src) to the never-cache external list, matching how
 * supabase/groq/anthropic are already handled. Previously these requests
 * fell through to the generic cache-first branch instead of always
 * hitting network.
 */
...
self.addEventListener('install', (event) => {
  ...
  // Intentionally no self.skipWaiting() here — the new worker should sit in
  // the "waiting" state so index.html can show the update banner and only
  // activate once the user confirms (applyUpdate() -> postMessage SKIP_WAITING
  // -> the message listener below calls self.skipWaiting()).
});
...
self.addEventListener('fetch', (event) => {
  ...
  const isExternal =
    ...
    url.hostname.includes('huggingface.co') || // covers huggingface.co, *.huggingface.co, cdn-lfs.huggingface.co
